...
function addNote(text = ''){
    const note = document.createElement('div');
    note.classList.add('note', 'addNote', 'i');
    note.innerHTML = `
        <div class="main ${text ? '' : 'hidden'}"></div>
        <textarea class="textarea ${text ? 'hidden' : ''}"></textarea>
        <div class="close">
    ...
    const open = note.querySelector('.open');
    const bold = note.querySelector('.bold');
    const children = Array.from(note.parentElement.children);
    ...
    note.addEventListener('click', (event) => {
        ...
        const allSiblings = Array.from(children).filter(
            (siblings) => siblings.classList.c !== fq.id
        );
