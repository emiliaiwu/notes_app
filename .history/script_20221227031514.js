...
function addNote(text = ''){
	...
	// 	// Toggle the display style of the open and close elements
	// 	main.classList.add('hidden');
	// 	textArea.classList.remove('hidden');
	// 	close.style.display = 'none';
	// 	open.style.display = 'flex';

	// 	// Get all siblings of the note element with a different id

	// 	const allSiblings = Array.from(event.target.parentNode.children).filter(
	// 		(sibling) => sibling.id !== elementId
	// 	);

	// 	// Iterate over the siblings and toggle the hidden class on the main and textArea elements
	...
	// Set up the event listener on the parent element
	note.addEventListener('click', function (event) {
		// Get the clicked element
        const clickedElement = event.target.parentElement;
        
        const close = clickedElement.querySelector('.close');
		const open = clickedElement.querySelector('.open');
		...
		const siblings = Array.from(clickedElement.parentNode.children).filter(
			(sibling) => sibling !== clickedElement
		);

		// Iterate over the siblings and reset their color to the default
