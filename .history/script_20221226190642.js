...
function addNote(text = ''){
    ...
    note.addEventListener('click', function (event){
        ...
        const allSiblings = Array.from(note.parentElement.children).filter(
            (sibling) => sibling.id !== note.id
        );


        if (note.parentElement.children.contains(event.target)) {
            // The click occurred within a sibling element
            // Return the siblings to the default state
            var siblings = note.parentNode.children;
